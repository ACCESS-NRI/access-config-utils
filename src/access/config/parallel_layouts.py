...
fixed counts, weights and free ranges do their work, and a division may leave cores idle -
...
Thread ranges travel down with the cores. A strategy that sets a ``thread_range``
replaces the range it inherited, for its component and everything below it, while the rest
inherit the root's. This is why the same component on the same cores is not always the same
question: a different inherited range gives genuinely different layouts, so the range is
recorded alongside the component, its core count and its strategy.
...
# result depends on: (component, n_cores, thread_range, strategy). ``total_cores`` is
...
class _LayoutSearch:
    ...
    against, and the subtree memo — while ``(component, n_cores, thread_range, strategy)``
    ...
    def run(self, component: ParallelComponent, strategy: RootAllocation) -> Iterator[ComponentLayout]:
        ...
        The root's ``thread_range`` seeds the inherited thread range, since there is nothing
        above it to inherit from.
        """
        return self._iter_component_layouts(component, self._total_cores, strategy.thread_range, strategy)
    ...
    def _cached_component_layouts(
        ...
        answer depends only on ``(component, n_cores, thread_range, strategy)``, not on what
        ...
        Args:
            component (ParallelComponent): The component to lay out.
            n_cores (int): Cores this component holds, to be spent or handed down.
            thread_range (tuple[int, int]): Inclusive ``(min, max)`` range of OpenMP threads
                per rank *inherited* from the enclosing strategy - *strategy*'s own override
                is applied downstream, not here. It is the inherited range that forms part
                of the memo key, since it is what the caller can vary.
            strategy (AllocationStrategy): The strategy node laid over *component*.

        Returns:
            tuple[ComponentLayout, ...]: Every valid layout, in enumeration order.
        """
        key = (component, n_cores, thread_range, strategy)
        layouts = self._cache.get(key)
        if layouts is None:
            layouts = tuple(self._iter_component_layouts(component, n_cores, thread_range, strategy))
            if not layouts:
                logger.debug(
    ...
    def _iter_component_layouts(
        ...
        The strategy's own ``thread_range``, when it sets one, replaces the range inherited
        ...
        Args:
            ...
            n_cores (int): Cores this component holds, to be spent or handed down.
            thread_range (tuple[int, int]): Inclusive ``(min, max)`` range of OpenMP threads
                per rank *inherited* from the enclosing strategy. This is the one place the
                range is still unresolved: *strategy* may override it, which is what
                ``resolve_thread_range`` settles before the candidates are built.
            strategy (AllocationStrategy): The strategy node laid over *component*.

        Yields:
            ComponentLayout: Each candidate that satisfies every local constraint of both
                *component* and *strategy*.
        """
        effective_thread_range = strategy.resolve_thread_range(thread_range)
        all_constraints = component.local_constraints + strategy.local_constraints

        if component.subcomponents:
            candidates = self._iter_branch_candidates(component, n_cores, effective_thread_range, strategy)
        else:
            candidates = _iter_leaf_candidates(component, n_cores, effective_thread_range)
        ...
        for candidate in candidates:
            ...
            if all_constraints and not all(c.is_satisfied(candidate) for c in all_constraints):
                continue
            yield candidate
    ...
    def _iter_branch_candidates(
        ...
        for core_split in core_splits:
            ...
            for combo in itertools.product(*sub_options):
                if all_group_constraints and not all(gc.is_satisfied(combo) for gc in all_group_constraints):
                    continue
...
def _iter_leaf_candidates(
    ...
    divides *n_cores*, giving ``n_cores // threads_per_rank`` ranks. Cores left idle are the
    business of the parent that chose not to hand them out.

    Args:
        component (ParallelComponent): The leaf component, whose domain - if it has one - is
            decomposed over the ranks each thread count implies.
        n_cores (int): Cores this component holds, spent exactly as ranks times threads.
        thread_range (tuple[int, int]): Inclusive ``(min, max)`` range of OpenMP threads per
            rank, already resolved for this component. Every count in it is tried, and the
            ones that do not divide *n_cores* are reported at ``DEBUG`` and skipped.

    Yields:
        ComponentLayout: One layout per (usable thread count, process grid) pair.
    """
    min_threads, max_threads = thread_range
    for threads_per_rank in range(min_threads, max_threads + 1):
        if n_cores % threads_per_rank != 0:
            logger.debug(
            ...
            continue
        n_ranks = n_cores // threads_per_rank
...
def _log_if_empty(
    ...
    Wrapping the whole search rather than checking after the fact keeps it lazy: a caller
    taking one layout and stopping never reaches the emptiness check at all.

    Args:
        layouts (Iterator[ComponentLayout]): The root's layouts, consumed lazily.
        component (ParallelComponent): The root component, named in the log message.
        total_cores (int): The whole budget, named in the log message.

    Yields:
        ComponentLayout: Each layout of *layouts*, unchanged.
    """
    found = False
...
def iter_layouts(
    ...
    *,
    ...
    and the rest inherit the root's.
    ...
    Args:
        ...
        allocations (RootAllocation | None): Allocation strategy for *component*, with one
            ...
            own because *component* always receives the whole budget; its ``thread_range``
            is the thread range every component inherits unless it overrides it, and its
