...
# ---------------------------------------------------------------------------
#
# This module never touches a real ParallelComponent: it reads ``.name`` and
# ``.subcomponents`` off whatever it is handed and nothing else, so a mock carrying those
# two is a faithful stand-in. ``subcomponents`` is a tuple, as the real type declares, so
# the mock cannot quietly accept a shape the real one would reject.


def component(name: str, *subcomponents: MagicMock) -> MagicMock:
    """A mock component tree node: a name, and the direct children below it."""
    comp = MagicMock()
    comp.name = name  # assigned, never passed: ``name=`` is reserved by Mock itself
...
class TestIterCoreSplits:
    ...
    def test_weighted_siblings_share_one_multiplier(self) -> None:
        strategy = FreeAllocation(subcomponents={"a": WeightedAllocation(weight=3), "b": WeightedAllocation(weight=2)})
...
class TestAllocationStrategy:
    ...
    def test_resolve_thread_range_prefers_its_own(self) -> None:
        assert FreeAllocation(thread_range=(2, 4)).resolve_thread_range((1, 1)) == (2, 4)

    def test_resolve_thread_range_falls_back_to_inherited(self) -> None:
        assert FreeAllocation().resolve_thread_range((1, 8)) == (1, 8)

    def test_invalid_strategy_thread_range_raises(self) -> None:
        with pytest.raises(ValueError, match="AllocationStrategy.thread_range maximum"):
            FreeAllocation(thread_range=(4, 2))

        with pytest.raises(ValueError, match="AllocationStrategy.thread_range minimum"):
            FreeAllocation(thread_range=(0, 2))
    ...
    def test_weight_valid(self) -> None:
        spec = WeightedAllocation(weight=3)
        assert spec.weight == 3

    def test_weight_zero_raises(self) -> None:
        with pytest.raises(ValueError, match=">= 1"):
            WeightedAllocation(weight=0)

    def test_weight_negative_raises(self) -> None:
        with pytest.raises(ValueError, match=">= 1"):
            WeightedAllocation(weight=-1)

    def test_equal_proportions_written_differently_enumerate_alike(self) -> None:
        # The point of reducing: only the ratio between siblings is a request, so no
        # spelling of it is privileged. Compared as whole enumerations, since a difference
        # in which budgets fit is exactly the bug this guards against.
        reduced = [WeightedAllocation(3), WeightedAllocation(2)]
        unreduced = [WeightedAllocation(6), WeightedAllocation(4)]
        for budget in range(1, 26):
            assert list(WeightedAllocation._iter_core_shares(reduced, budget)) == list(
                WeightedAllocation._iter_core_shares(unreduced, budget)
            )

    def test_an_unreduced_proportion_fits_the_budget_its_reduction_fits(self) -> None:
        # Six cores cannot be split 6:4 as written, but that is not what 6:4 asks for.
        allocs = [WeightedAllocation(6), WeightedAllocation(4)]
        assert list(WeightedAllocation._iter_core_shares(allocs, 6)) == [(3, 2)]

    def test_a_lone_weight_is_not_reduced(self) -> None:
        # With no sibling to be in proportion to there is nothing to reduce against, and
        # reducing it alone would leave the weight meaning nothing: (1,), (2,), ... instead.
        allocs = [WeightedAllocation(2)]
        assert list(WeightedAllocation._iter_core_shares(allocs, 5)) == [(2,), (4,)]
    ...
    def test_with_local_constraints(self) -> None:
        # Which constraint it is does not matter: the field only has to hold it unchanged.
        c = Mock(spec=LocalConstraint)
...
class TestRootAllocation:
    ...
    def test_it_keeps_the_fields_a_root_may_set(self) -> None:
        root = RootAllocation(thread_range=(1, 4), subcomponents={"a": FreeAllocation()})
        assert root.thread_range == (1, 4)
...
class TestFractionalBounds:
    ...
    def test_a_mode_that_cannot_hold_a_fraction_is_left_as_it_is(self) -> None:
        weighted = WeightedAllocation(weight=3)
        assert weighted._resolve_fractions(400) is weighted
...
class TestResolveRootStrategy:
    ...
    def test_a_valid_strategy_is_returned_unchanged(self) -> None:
        comp = component("r", component("a"))
        given = RootAllocation(subcomponents={"a": FixedAllocation(n_cores=4)})
        assert resolve_root_strategy(comp, given, 16) is given

    @pytest.mark.parametrize(
        "allocations",
        [FixedAllocation(n_cores=4), WeightedAllocation(weight=2), FreeAllocation(max_cores=8), FreeAllocation()],
    )
    ...
    def test_fractions_are_resolved_against_the_budget(self) -> None:
        ...
        comp = component("r", component("a"), component("b"))
...
class TestSchedulingProtocol:
    ...
    def test_phases_are_served_in_order_regardless_of_position(self) -> None:
        # Declaration order is free, weighted, fixed; the schedule is still fixed,
        # weighted, free.
        strategy = FreeAllocation(
            subcomponents={
                "f": FreeAllocation(min_cores=1),
                "r": WeightedAllocation(weight=2),
    ...
    def test_a_middle_phase_reserves_nothing_by_default(self) -> None:
        # Fixed, then weighted, then free: computing what fixed must leave behind calls
        # the base-class _reserved_cores for the weighted group, which reserves nothing.
        strategy = FreeAllocation(
            subcomponents={
                "x": FixedAllocation(n_cores=2),
                "r": WeightedAllocation(weight=1),
                "f": FreeAllocation(min_cores=1),
            }
        )
